...
// ------------------------------------------------ 归属证据（§6.2.1；原 NC-3「唯一同名命中」）
//
// 本组原以「唯一同名命中 ≠ 归属证据」命名，但实测（隔离副本 NC-B2）该表述与实判守点
// 长期脱节：真正被 `hasOwnershipEvidence` 判别的是「**有既有条目** + 无证据/失效」
// 这一类（本组 `M5：本区「有既有条目但无历史证据」…` 与 `…带 ephemeral…` 两条），
// 而「无既有条目」那一格由 M5 的不造条目分支拦下、不经证据位。组名据实收敛。
...
describe('DocIndex · 归属证据（§6.2.1 / NC-3 期望）', () => {
  ...
  it('M5：唯一同名 + 无既有条目：不建条目进历史池（凭文件名不能造 ws: 身份）', () => {
    // **N-4 收口：本用例已改名**（原名「唯一同名命中不自动绑定，进历史池」）。
    //
    // 为什么改名（实测，非猜测）：改名前的名字所指的守点是「**归属证据**不足 → 不认领」，
    // 但本用例的绿色**不来自**证据位。实测把 `hasOwnershipEvidence` 中性化成恒 `true`
    // 后，本用例**仍然通过**（隔离副本 NC-B2：`Tests 4 failed | 56 passed`，
    // 4 条红里没有它）——因为本场景**没有既有条目**，M5 在查证据位**之前**就落进
    // 「无 existing → 历史池」那一支（`docIndexMigrate.ts:139-172`），证据位根本不被消费。
    // 即名字与实判守点**仍然脱节**，故按主控给的选项 ① 改名使名实相符。
    //
    // 名字所指的守点（证据不足 → 不得认领）现由这两条覆盖：
    //   - `M5：本区「有既有条目但无历史证据」→ 不得认领，进历史池`（负控 b）
    //   - `M5：既有条目带 ephemeral（上次会话身份已失效）→ 不得认领，进历史池`
    // 二者在 `hasOwnershipEvidence` 中性化后**都会转红**（见隔离副本 NC-B2 的 4 条红）。
