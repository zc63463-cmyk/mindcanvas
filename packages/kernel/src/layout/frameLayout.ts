 ...
 *
 * ## S3 卫星（`LayoutResult.satellites`，S3-R1 frame 修复）
 *
 * **契约：展开对加法字段必须显式表态——不得静默丢弃。**
 *
 *  - **框内**摘要（摘要节点在某个框根的后代里）：在基座上**本来就不会**产出卫星。
 *    框根整棵后代被 `framePrunedCollapsed` 剪掉，基座的卫星钩子看不到任何框内成员盒
 *    （`satellite.ts` 的 `bandOfMembers` 无盒 → 不产出卫星），而框岛重建路径没有、也
 *    不应凭空再造一套卫星几何（那要把卫星放置接进框岛的局部坐标 / 岛级缓存体系，超出
 *    本批安全范围）。故框内摘要在展开结果里**留在普通流行内可见**（降级，不消失）——
 *    这正是 `LayoutResult.satellites` 头注释写明的「框内 = 该布局路径不支持卫星」。
 *  - **框外**摘要（摘要节点不在任何框根的后代里）：与框无关，基座布局能正常为它产出
 *    卫星（成员盒在基座上可见）。展开只重建框内子树，**不碰**这些卫星——故必须把
 *    `base.satellites` **原样传播**（同一批对象引用；这些卫星不在 `ctx.dropped` 里，
 *    几何与世界坐标都不受岛展开影响）。不传播就是「加法字段静默丢失」：
 *    `pipeline.layoutDemo` 的框路径会丢掉框外摘要的括线位置。
 *
 * 两个分支都由下面的实现显式写出：无卫星时字段**缺失**（与基线逐位等价），
 * 有卫星时字段**携带同一批引用**。没有「有时传播、有时不传播」的中间状态。
 *
...
export function expandFrameIslands(
  ...
  // S3 卫星：**显式表态**，不得静默丢弃（契约见函数头「S3 卫星」）。
  // 框外摘要的卫星不在 ctx.dropped 里（它不是任何框根的后代），岛展开零触碰其几何
  // （世界坐标、缓存条目均未变），故原样传播**同一批对象引用**；框内摘要本就不产出
  // 卫星 → 数组为空 → 字段缺失（无摘要文档与基线逐位等价的既有契约由此保持）。
