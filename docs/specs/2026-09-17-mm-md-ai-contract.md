...
2. **根节点笔记块**里的 `centers` / `cid` / `next_cid` / `sections` — 多岛与分区；**节点级** `summary_of` — 概要括线（协议 §5–§6.3–§6.4–§6.5）。  
...
- 需要概要时写**摘要节点自己**的 `summary_of: {from: cid:…, to: cid:…}`（协议 §6.5）：成员必须是**同一个父下的连续兄弟区间**且**含两端**；`from == to` = 单成员概念仍成立。两字段都要写，`cid:` 优先。  
  ⚠️ **不要**把摘要素材写进成员区间内（成员区间含另一摘要 = 不摘除、无括线）；也**不要**指望它在 org / 显式 `note.dir` / 框内布局下画括线（协议 §6.5「支持边界」，那些路径留流内可见）。
...
|---|---|
| `.mm.md` 文件 | **主通道** |
| Centers / edges / sections / summary_of | 按本契约读写 |
